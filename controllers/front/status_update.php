...
class IDcutStatus_UpdateModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        if (isset($_SERVER['CONTENT_TYPE']) && $_SERVER['CONTENT_TYPE'] === 'application/json') {
            ...
            if (!is_array($data) || !isset($data['params']['transaction']['id'])
                || !Validate::isReference($data['params']['transaction']['id'])) {
                echo 'bad request';
                exit;
            }
    ...
    public function processUpdate(IDcutTransaction $IDcutTransaction)
    {
        ...
        switch ($transaction->getStatus()) {
            case 'completed':
                $orderStatusUpdate = $this->setOrderStatus(Configuration::get('PS_OS_PAYMENT'),
                    $IDcutTransaction->order, null);
                break;
            case 'pending':
                $orderStatusUpdate = $this->setOrderStatus(Configuration::get('PS_OS_IDCUT_PENDING'),
                    $IDcutTransaction->order, null);
                break;
            ...
            case 'cancelled_by_user':
                $orderStatusUpdate = $this->setOrderStatus(Configuration::get('PS_OS_CANCELED'),
                    $IDcutTransaction->order, null);
                break;
            case 'cancelled_by_payment_gateway':
                ...
                break;
            case 'error':
                $orderStatusUpdate = $this->setOrderStatus(Configuration::get('PS_OS_ERROR'),
                    $IDcutTransaction->order, null);
                break;
            default:
                $orderStatusUpdate = $this->setOrderStatus(Configuration::get('PS_OS_IDCUT_PENDING'),
    ...
    protected function setOrderStatus($new_id_order_state, $order,
                                      $message = null)
    {
            ...
            if ($current_order_state->id != $order_state->id) {
                ...
                if ($history->id_order_state == Configuration::get('PS_OS_SHIPPING')
                    && $order->shipping_number)
                        $templateVars = array('{followup}' => str_replace('@',
                            $order->shipping_number, $carrier->url));
